...
*Response:* Using the instrumental variable approach addresses selection bias in wind turbine location. The threats to being able to confidently identify causal effect are the potential effect of political boundaries on the location of proposed wind turbines. Additionally mentioned in the paper, as potential selection biases is average home price, population with a university degree, median income, population density, and distance to lakes are not determining where wind turbines are proposed separate from wind power. 
...
```{r}
first_stage <- lm(proposed_turbine_3km ~ log_wind_power + mindistlake + mindistlake_sq + longitude + long_sq + latitude + lat_sq + long_lat + as.factor(district_id), # fixed effect only on location
                  data = matched_data)
...
```{r}
second_stage <- lm(change_liberal ~ proposed_turbine_3km_HAT + mindistlake + mindistlake_sq + longitude + long_sq + latitude + lat_sq + long_lat + as.factor(district_id),
                  data = matched_data)
...
fit_2sls <- ivreg(change_liberal ~ proposed_turbine_3km + mindistlake + mindistlake_sq + longitude + long_sq + latitude + lat_sq + long_lat | log_wind_power + mindistlake + mindistlake_sq + longitude + long_sq + latitude + lat_sq + long_lat,
                  data = matched_data)
